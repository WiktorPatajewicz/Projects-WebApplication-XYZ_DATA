...
<head>
    ...
    <body>
        ...
        <div id="container">
            ...
            <div id="content">
                <span class="bigtitle">Usuwanie spotkań</span>
                <div style="height: 15px;"></div>

                <div id="form2">
                    <h3><b>Wyszukaj ID spotkania, które chcesz usunąć:</b></h3>
                    <form method="POST">
                        <input type="text" name="szukajInput" maxlength="10" placeholder="1">
                ...
                <?php
                    if(isset($_POST['szukaj'])){
                        ...
                        $sql_all = "SELECT * FROM spotkanie";
                        $query_all = mysqli_query($db, $sql_all);
                        $searchInput = $_POST['szukajInput'];
                        if($db){
                            $sql_search =  "SELECT * FROM spotkanie WHERE id_spotkanie = '$searchInput'";
                            $query_search = mysqli_query($db, $sql_search);
        
                            if(empty($searchInput)){
                                echo "<br>";
                                echo "<b>Wpisz ID spotkania</b>";
                            }else if (mysqli_num_rows($query_search) == 0){
                                echo "<br>";
                                echo "<b>Takie spotkanie nie istnieje</b>";
                            }else {
                                echo "<br>";
                                echo"<table>";
                                    echo "<tr>";
                                        echo "<td>" . '<b>ID Spotkania</b>'. "</td>";
                ...
                ?>
                <br><br>
                <h3><b>Wpisz ID spotkania, które chcesz usunąć:</b></h3>
                <form method="POST">
                    <input type="text" name="id_spotkanie" maxlength="10" placeholder="1">
                ...
                <?php
                    if (isset($_POST['Usun'])){
                        ...
                        if($db){
                            $id = mysqli_real_escape_string($db, $_POST['id_spotkanie']);
                            $zapytanie = "DELETE FROM spotkanie WHERE id_spotkanie =". $_POST['id_spotkanie'];
                            $query = mysqli_query($db, $zapytanie);

                            if (empty($id)){
                                
                                echo "<B>Podaj ID spotkania do usunięcia</b>";
                            }else{ 
                                echo "<b>Spotkanie zostało usunięte</b>";
                            }
                        }else{
                            echo "<b>Błąd połączenia z bazą danych</b>";
                        }
